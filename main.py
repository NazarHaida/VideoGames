...
with open('models/model_JP.joblib', 'rb') as file:
    model_jp= load(file)

with open('models/model_NA.joblib', 'rb') as file:
    model_na = load(file)
...
with open('models/model_EU.joblib', 'rb') as file:
    model_eu = load(file)
# model_eu.predict()
if hasattr(model_eu, 'feature_importances_'):
    print("Model expects the following features (in order of importance):")
    print(model_eu.feature_importances_)
else:
    print("The model object does not contain feature importances.")
with open('category_mappings.json', 'r') as f:
    category_mapping = json.load(f)
def map_categories(game_data, category_mappings):

    for key, mapping in category_mappings.items():
        if key in game_data and game_data[key] in mapping:
            game_data[key] = mapping[game_data[key]]
        else:
            game_data[key] = -1  # Default mapping for unknown categories
    return game_data
...
def add_game():
    if request.method == 'POST':
        ...
        key_mapping = {
            'name':'Name',
            'yearOfRelease': 'Year_of_Release', 'yearOfPred': 'Year_of_Prediction','criticScore': 'Critic_Score',
            ...
            'rating': 'Rating_E', 'gameCount': 'Game_Count', 'euSalesPerc':'EU_Sales_Percent','naSalesPerc':'NA_Sales_Percent','jpSalesPerc':'JP_Sales_Percent','otherSalesPerc':'Other_Sales_Percent',
            'genre': "Genre", 'rankEuDeveloper': 'Rank_EU_Developer','rankNaDeveloper': 'Rank_NA_Developer', 'rankOtherDeveloper': 'Rank_Other_Developer','rankJpDeveloper': 'Rank_JP_Developer','steamId': 'steamId', 'currency': 'currency','developerRank':'Developer_Rank'
        }
        name = game_data.get("name")
        platform = game_data.get("platform")
        year_of_release = game_data.get("yearOfRelease")
        year_of_pred = game_data.get("yearOfPred")
        ...
        def calculate_game_age(year_of_release):
            return int(year_of_pred) - int(year_of_release) if year_of_release else 0
        ...
        game_data_df['Rank_NA_Developer'] = pd.to_numeric(game_data_df['Rank_NA_Developer'], errors='coerce')
        game_data_df['Rank_JP_Developer'] = pd.to_numeric(game_data_df['Rank_JP_Developer'], errors='coerce')
        game_data_df['Rank_Other_Developer'] = pd.to_numeric(game_data_df['Rank_Other_Developer'], errors='coerce')
        ...
        print("Ranks:", game_data_df['Rank_EU_Developer'],game_data_df['Developer_Rank'])

        game_data_df['Rank_NA_Developer'] = int(pd.cut(
            game_data_df['Rank_NA_Developer'], bins=loaded_bin_edges['NA'], labels=False, include_lowest=True
        ) + 1)

        game_data_df['Rank_Other_Developer'] = int(pd.cut(
            game_data_df['Rank_Other_Developer'], bins=loaded_bin_edges['Other'], labels=False, include_lowest=True
        ) + 1)

        game_data_df['Rank_JP_Developer'] = int(pd.cut(
            game_data_df['Rank_JP_Developer'], bins=loaded_bin_edges['JP'], labels=False, include_lowest=True
        ) + 1)

        model_columns = [
            'Platform', 'Genre', 'Publisher', 'Critic_Score', 'Critic_Count', 'User_Score', 'User_Count',
            'Developer', 'NA_Sales_Percent', 'EU_Sales_Percent', 'JP_Sales_Percent', 'Other_Sales_Percent','Game_Count', 'Developer_Rank',  'Rank_NA_Developer','Rank_EU_Developer',  'Rank_JP_Developer',  'Rank_Other_Developer','Game_Age', 'Rating_E'
        ]

        game_data = {k: game_data_df[k] for k in model_columns if k in game_data_df}
        ...
        try:
            global_predict = model_eu.predict(input_df)
            print("EU Sales Prediction:", global_predict)
        ...
        price = global_predict[0] * 1000000 * price_per_game
        ...
        return jsonify({
            ...
            'predicted_eu_sales': "%.2f"%global_predict[0],
...
def add_game_eu():
    if request.method == 'POST':
        ...
        key_mapping = {
            ...
            'criticCount': 'Critic_Count', 'userScore': 'User_Score', 'userCount': 'User_Count',
            'platform': 'Platform', 'publisher': 'Publisher', 'developer': 'Developer',
            'rating': 'Rating_E', 'gameCount': 'Game_Count', 'euSalesPerc':'EU_Sales_Percent',
        ...
        model_columns = [
            'Platform', 'Genre', 'Publisher', 'Critic_Score', 'Critic_Count', 'User_Score', 'User_Count',
            'Developer', 'EU_Sales_Percent','Game_Count', 'Developer_Rank', 'Rank_EU_Developer',  'Game_Age', 'Rating_E'
        ]
...
def add_game_na():
    if request.method == 'POST':
        ...
        key_mapping = {
            ...
            'criticCount': 'Critic_Count', 'userScore': 'User_Score', 'userCount': 'User_Count',
            'platform': 'Platform', 'publisher': 'Publisher', 'developer': 'Developer',
            'rating': 'Rating_E', 'gameCount': 'Game_Count', 'naSalesPerc':'NA_Sales_Percent',
            'genre': "Genre", 'rankNaDeveloper': 'Rank_NA_Developer', 'steamId': 'steamId', 'currency': 'currency','developerRank':'Developer_Rank'
        }
        name = game_data.get("name")
        platform = game_data.get("platform")
        year_of_release = game_data.get("yearOfRelease")
        year_of_pred = game_data.get("yearOfRelease")
        genre = game_data.get("genre")
        publisher_name = game_data.get("publisherName")
        # na_sales = game_data.get("naSales")
        # eu_sales = game_data.get("euSales")
        ...
        model_columns = [
            'Platform', 'Genre', 'Publisher', 'Critic_Score', 'Critic_Count', 'User_Score', 'User_Count',
            'Developer', 'NA_Sales_Percent','Game_Count', 'Developer_Rank', 'Rank_NA_Developer',  'Game_Age', 'Rating_E'
        ]
        ...
        try:
            na_predict = model_eu.predict(input_df)
            print("NA Sales Prediction:", na_predict)
        except Exception as e:
            print("Error in prediction:", e)
            return jsonify({'error': 'Error during prediction'}), 500
        steam_id = game_data_df['steamId'].iloc[0]
        ...
        price = na_predict[0] * 1000000 * price_per_game
        ...
        insert_ratings_query = """
               INSERT INTO ratings (Critic_Score, Critic_Count, User_Score, User_Count, Developer, Rating, Game_Count, Developer_Rank, Rank_NA_Developer) 
               VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
               """
        cursor.execute(insert_ratings_query, (
            critic_score, critic_count, user_score, user_count, developer, rating, gameCount, developer_rank, na_sales_rank
        ))
        cnx.commit()
        ...
        return jsonify({
            'message': 'Game and related data added successfully!',
            'predicted_eu_sales': "%.2f"%na_predict[0],
            'calculated_price': (int(price), currency)
        }), 201
...
def add_game_jp():
    if request.method == 'POST':
        ...
        key_mapping = {
            ...
            'criticCount': 'Critic_Count', 'userScore': 'User_Score', 'userCount': 'User_Count',
            'platform': 'Platform', 'publisher': 'Publisher', 'developer': 'Developer',
            'rating': 'Rating_E', 'gameCount': 'Game_Count', 'jpSalesPerc':'JP_Sales_Percent',
            'genre': "Genre", 'rankEuDeveloper': 'Rank_JP_Developer', 'steamId': 'steamId', 'currency': 'currency','developerRank':'Developer_Rank'
        }
        name = game_data.get("name")
        platform = game_data.get("platform")
        year_of_release = game_data.get("yearOfRelease")
        ...
        model_columns = [
            'Platform', 'Genre', 'Publisher', 'Critic_Score', 'Critic_Count', 'User_Score', 'User_Count',
            'Developer', 'JP_Sales_Percent','Game_Count', 'Developer_Rank', 'Rank_JP_Developer',  'Game_Age', 'Rating_E'
        ]
        ...
        try:
            jp_predict = model_eu.predict(input_df)
            print("EU Sales Prediction:", jp_predict)
        except Exception as e:
            print("Error in prediction:", e)
            return jsonify({'error': 'Error during prediction'}), 500
        steam_id = game_data_df['steamId'].iloc[0]
        ...
        price = jp_predict[0] * 1000000 * price_per_game
        ...
        return jsonify({
            'message': 'Game and related data added successfully!',
            'predicted_eu_sales': "%.2f"%jp_predict[0],
            'calculated_price': (int(price), currency)
        }), 201
...
def add_game_other():
    if request.method == 'POST':
        ...
        key_mapping = {
            ...
            'criticCount': 'Critic_Count', 'userScore': 'User_Score', 'userCount': 'User_Count',
            'platform': 'Platform', 'publisher': 'Publisher', 'developer': 'Developer',
            'rating': 'Rating_E', 'gameCount': 'Game_Count', 'otherSalesPerc':'Other_Sales_Percent',
            'genre': "Genre", 'rankOtherDeveloper': 'Rank_Other_Developer', 'steamId': 'steamId', 'currency': 'currency','developerRank':'Developer_Rank'
        }
        name = game_data.get("name")
        platform = game_data.get("platform")
        year_of_release = game_data.get("yearOfRelease")
        ...
        model_columns = [
            'Platform', 'Genre', 'Publisher', 'Critic_Score', 'Critic_Count', 'User_Score', 'User_Count',
            'Developer', 'Other_Sales_Percent','Game_Count', 'Developer_Rank', 'Rank_Other_Developer',  'Game_Age', 'Rating_E'
        ]
        ...
        try:
            other_predict = model_other.predict(input_df)
            print("Other Sales Prediction:", other_predict)
        except Exception as e:
            print("Error in prediction:", e)
            return jsonify({'error': 'Error during prediction'}), 500
        steam_id = game_data_df['steamId'].iloc[0]
        ...
        price = other_predict[0] * 1000000 * price_per_game
        ...
        return jsonify({
            'message': 'Game and related data added successfully!',
            'predicted_eu_sales': "%.2f"%other_predict[0],
            'calculated_price': (int(price), currency)
        }), 201
